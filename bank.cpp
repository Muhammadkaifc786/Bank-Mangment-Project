...
public:
    ...
    void depositAmount(string id,int amount)
   {
    ...
    void saveToFile() const {
        ofstream outFile("bank_data.txt", ios::app);  // Open file in append mode
        if (!outFile.is_open()) {
...
int main() {
  ...
  if (choice == 1) {
    ...
    if (choice_Account_Type == 1) {
        ...
        // Save data to file after adding an account
        bank.saveToFile();
